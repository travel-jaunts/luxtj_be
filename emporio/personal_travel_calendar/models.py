...
class UserPersonalTravelCalendar(BaseDataModel):
    """
    ...
    is_date_range: Mapped[bool] = mapped_column(
        nullable=False,
        default=False,
        comment="Indicates if the occasion is a date range",
    )
    ...
    holiday_mood_preference: Mapped[str] = mapped_column(
        nullable=False,
        comment="User's mood preferences for holiday",
    ...
    holiday_theme_preference: Mapped[HolidayThemePreference] = mapped_column(
        nullable=False,
        comment="User's theme preferences for holiday",
        default=[],
    )
    __table_args__ = (
        CheckConstraint(
