...
class OffersKpiSummarySerializer(ApiSerializerBaseModel):
    ...
    def from_domain_model(
        ...
        return cls(
            total_discount_given=AmountSerializer(
                amount=offers_kpi_summary.total_discount_amount, currency=offers_kpi_summary.amount_currency
            ),
            ...
            net_revenue_after_discount=AmountSerializer(
                amount=offers_kpi_summary.net_revenue_after_discount, currency=offers_kpi_summary.amount_currency
            ),
        )
...
class OfferLineItemSerializer(ApiSerializerBaseModel):
    ...
    applicable_on: OfferApplicabilityEnum       # conditions (coupon code, user segment, payment method, etc.)
    min_booking_amount: AmountSerializer
    max_discount_cap: float             # for percentage discounts, this is the maximum discount amount that can be applied
    per_user_limit: int
    stackable: bool
    usage_count: int
    total_discount_given: AmountSerializer
    ...
    def from_domain_model(cls, offer: OfferDomainModel) -> "OfferLineItemSerializer":
        return cls(
            ...
            min_booking_amount=AmountSerializer(amount=offer.min_booking_amount, currency=offer.offer_currency),
            max_discount_cap=offer.max_discount_cap,
            per_user_limit=offer.per_user_limit,
            stackable=offer.stackable,
            usage_count=offer.usage_count,
            total_discount_given=AmountSerializer(amount=offer.total_discount_given, currency=offer.offer_currency),
